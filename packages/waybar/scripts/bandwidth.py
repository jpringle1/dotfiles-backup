...
def format_size_mbit(size):
    """ Always show speed in mb: number of bytes divided by (2^10 * 2^10 / 8)"""
    return f"{size / 2**17:3.0f}Mb"

def format_size_dynamic(size):
...
def main():
    ...
    while True:
        ...
        rx_bytes, tx_bytes = get_rx_tx_bytes(iface)
        drx = format_size_mbit((rx_bytes - prev_rx_bytes) / refresh_interval)
        dtx = format_size_mbit((tx_bytes - prev_tx_bytes) / refresh_interval)
